...
public class TimeSeriesCollectionTest {
    ...
    public void testFindDomainBounds() {
        ...
        // no series, no visible keys -> null domain
        Range r = DatasetUtilities.findDomainBounds(dataset, visibleSeriesKeys, true);
        assertNull(r);

        // S1 visible but empty -> still null domain
        TimeSeries s1 = new TimeSeries("S1");
        dataset.addSeries(s1);
        ...
        // add one year to S1 -> non-null, ordered domain
        s1.add(new Year(2008), 8.0);
        r = DatasetUtilities.findDomainBounds(dataset, visibleSeriesKeys, true);
        assertNotNull(r);
        assertTrue(r.getLowerBound() <= r.getUpperBound());

        double lowerWithOnlyS1 = r.getLowerBound();
        double upperWithOnlyS1 = r.getUpperBound();

        // add S2 with 2009 and 2010, but keep only S1 visible -> bounds unchanged
        TimeSeries s2 = new TimeSeries("S2");
        dataset.addSeries(s2);
        ...
        r = DatasetUtilities.findDomainBounds(dataset, visibleSeriesKeys, true);
        assertNotNull(r);
        assertEquals(lowerWithOnlyS1, r.getLowerBound(), 0.0);
        assertEquals(upperWithOnlyS1, r.getUpperBound(), 0.0);

        // make S2 visible as well -> upper bound must increase
        visibleSeriesKeys.add("S2");
        r = DatasetUtilities.findDomainBounds(dataset, visibleSeriesKeys, true);
        assertNotNull(r);
        assertTrue(r.getLowerBound() <= r.getUpperBound());
        assertEquals(lowerWithOnlyS1, r.getLowerBound(), 0.0);
        assertTrue("Upper bound should increase when S2 is visible",
                r.getUpperBound() > upperWithOnlyS1);
    }
